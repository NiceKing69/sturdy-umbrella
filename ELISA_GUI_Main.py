...
class ELISAGUI:
    def __init__(self, master):
        ...
        self.data = None
        self.standard_conc_cells = []    # coordinates of standard concentration cells
        self.standard_signal_cells = []  # coordinates of standard signal cells
        self.sample_name_cells = []      # coordinates of sample name cells
        self.sample_signal_cells = []    # coordinates of sample signal cells
        ...
        tk.Radiobutton(frame_type, text="Normal ELISA",
                       variable=self.assay_type, value="normal").pack(side=tk.LEFT)
        tk.Radiobutton(frame_type, text="Competitive ELISA",
                       variable=self.assay_type, value="competitive").pack(side=tk.LEFT)
        ...
        tk.Radiobutton(frame_pres, text="Present",
                       variable=self.sample_presence, value="present").pack(side=tk.LEFT)
        tk.Radiobutton(frame_pres, text="Not Present",
                       variable=self.sample_presence, value="not_present").pack(side=tk.LEFT)
        ...
        self.load_btn = tk.Button(
            master,
            text="Start with Loading Excel File",
            command=self.load_file)
        self.load_btn.pack(pady=5)

        self.choose_standard_conc_btn = tk.Button(
            master,
            text="Choose Standard Concentration",
            command=self.set_standard_conc_mode,
            state=tk.DISABLED)
        self.choose_standard_conc_btn.pack(pady=2)

        self.choose_standard_btn = tk.Button(
            master,
            text="Choose Standard Signal",
            command=self.set_standard_signal_mode,
            state=tk.DISABLED)
        self.choose_standard_btn.pack(pady=2)

        self.choose_sample_name_btn = tk.Button(
            master,
            text="Choose Sample Name",
            command=self.set_sample_name_mode,
            state=tk.DISABLED)
        self.choose_sample_name_btn.pack(pady=2)

        self.choose_sample_btn = tk.Button(
            master,
            text="Choose Sample Signal",
            command=self.set_sample_signal_mode,
            state=tk.DISABLED)
        self.choose_sample_btn.pack(pady=2)

        self.fit_btn = tk.Button(
            master,
            text="Run Analysis & Export",
            command=self.fit_and_export,
            state=tk.DISABLED)
    ...
    def load_file(self):
        ...
        # Enable all selection and analysis controls
        for btn in (
    ...
    def set_standard_conc_mode(self):
        """
        Enter mode to select the standard concentration cells.
        User must select exactly one row or one column and press Enter.
        """
    ...
    def set_standard_signal_mode(self):
        """
        Enter mode to select the standard signal cells.
        Requires that concentration cells have been selected first.
        """
    ...
    def set_sample_name_mode(self):
        """
        Enter mode to select the sample name cells.
        """
    ...
    def set_sample_signal_mode(self):
        """
        Enter mode to select the sample signal cells.
        Requires that sample name cells have been selected first.
        """
    ...
    def confirm_selection(self, event):
        """
        Handle the Enter key: verify the selection is a single row or column,
        highlight the cells, and store their coordinates for later analysis.
        """
        ...
        # Determine if user selected a single row or a single column
        rows = {r for r,c in sel}
        cols = {c for r,c in sel}
        if not (len(rows) == 1 or len(cols) == 1):
            messagebox.showerror(
                "Error",
                "Select exactly one row or one column."
            )
            return

        # Highlight selection with mode-specific color
        mode = self.selecting_mode
        color_map = {
            'conc':      'orange',
            'std_sig':   'lightblue',
            'samp_name': 'yellow',
        ...
        color = color_map.get(mode, 'grey')

        for r, c in sel:
            self.sheet.highlight_cells(row=r, column=c, bg=color)

        # Store coordinates for later use
        if mode == 'conc':
            self.standard_conc_cells = sel.copy()
        elif mode == 'std_sig':
            self.standard_signal_cells = sel.copy()
        elif mode == 'samp_name':
            self.sample_name_cells = sel.copy()
        elif mode == 'samp_sig':
            self.sample_signal_cells = sel.copy()

        # Reset prompt and unbind
        self.prompt_label.config(text='')
        self.selecting_mode = None
        self.sheet.unbind("<Return>")

    # --- Model functions for different analysis methods ---
    ...
    def inverse_4pl(self, y, A, B, C, D):
        """
        Invert the 4PL curve to estimate concentration from signal.
        """
        try:
            ratio = (A - D) / (y - D) - 1
            if ratio <= 0:
    ...
    def fit_and_export(self):
        """
        Perform the selected analysis on the standard points,
        optionally predict sample concentrations,
        display tables, plot the curve (with R, R²), and export to Excel.
        """
        try:
            # Verify standard selections
            if not self.standard_conc_cells or not self.standard_signal_cells:
                raise ValueError("Standard concentrations or signals not set.")

            # Build mapping of concentration -> signal values
            std_map = {}
            for r, c in self.standard_signal_cells:
                # find matching concentration cell in same row/col
                conc = None
                for rr, cc in self.standard_conc_cells:
                    if rr == r or cc == c:
                        conc = float(self.data.iat[rr, cc])
                        break
                if conc is None:
                    raise ValueError(f"No matching concentration for signal at ({r},{c})")
                std_map.setdefault(conc, []).append(float(self.data.iat[r, c]))

            # Prepare arrays for fitting
            concs = np.array(sorted(std_map.keys()))
            ...
            if method == '4PL':
                popt, pcov = curve_fit(
                    self.four_param_logistic, all_c, all_s, maxfev=10000
                )
                model_func = self.four_param_logistic
            elif method == '5PL':
                popt, pcov = curve_fit(
                    self.five_param_logistic, all_c, all_s, maxfev=10000
                )
                ...
                m, b = np.polyfit(
                    np.log(all_c[mask]), np.log(all_s[mask]), 1
                )
                popt = np.array([m, b]); pcov = None
            ...
            # Parameter errors
            perr = (
                np.sqrt(np.diag(pcov)) if pcov is not None
                else [np.nan] * len(popt)
            )

            # Goodness of fit
            y_pred = model_func(all_c, *popt)
            r2 = 1 - np.sum((all_s - y_pred)**2) / np.sum((all_s - np.mean(all_s))**2)
            r  = np.sqrt(r2) if r2 >= 0 else np.nan
            rmse = np.sqrt(np.mean((all_s - y_pred)**2))
            ...
            # Predict sample concentrations if selected
            have_samples = bool(
                self.sample_name_cells and self.sample_signal_cells
            )
            if have_samples:
                # Extract sample names & signals
                sample_names   = [str(self.data.iat[r, c]) for r, c in self.sample_name_cells]
                sample_signals = [float(self.data.iat[r, c]) for r, c in self.sample_signal_cells]

                if self.sample_presence.get() == 'present':
                    # Use inverse model for each sample
                    sample_concs = []
                    for sig in sample_signals:
                        if method in ['4PL', '5PL']:
                            sample_concs.append(
                                self.inverse_4pl(sig, *popt)
                            )
                        elif method == 'Linear':
                            sample_concs.append((sig - popt[1]) / popt[0])
                        else:
                            sample_concs.append(
                                np.exp((np.log(sig) - popt[1]) / popt[0])
                            )
                else:
                    sample_concs = [np.nan] * len(sample_signals)

                # Display sample estimates in a pop-up table
                df_s = pd.DataFrame({
                    "Sample Name":           sample_names,
                    "Signal":                sample_signals,
                    "Estimated Concentration": sample_concs
                })
                win1 = tk.Toplevel(self.master)
                win1.title("Sample Estimates")
                sh1 = Sheet(win1, width=500, height=200)
                sh1.pack(fill=tk.BOTH, expand=True)
                sh1.set_sheet_data(
                    [df_s.columns.tolist()] + df_s.values.tolist()
                )

            # Prepare parameters table
            if method == '5PL':
                labels = ['A', 'B', 'C', 'D', 'E']
            elif method in ['Linear', 'Log-Linear']:
                labels = ['m', 'b']
            else:
                labels = ['A', 'B', 'C', 'D']

            params = [
                [labels[i], popt[i], perr[i] if i < len(perr) else ""]
            ...
            params += [
                ...
                ["RMSE", rmse, ""],
                ["LoD Signal", lod_sig, ""],
                ["LoD Concentration", lod_conc, ""]
            ]

            df_p = pd.DataFrame(params, columns=["Parameter", "Estimate", "Std Error"])
            win2 = tk.Toplevel(self.master)
            win2.title("Parameters & LoD")
            sh2 = Sheet(win2, width=400, height=250)
            sh2.pack(fill=tk.BOTH, expand=True)
            sh2.set_sheet_data(
                [df_p.columns.tolist()] + df_p.values.tolist()
            )
            ...
            if have_samples and self.sample_presence.get() == 'present':
                plt.scatter(sample_concs, sample_signals, label='Samples')
            plt.plot(xf, yf, label=method)
            plt.axhline(lod_sig, linestyle='--', label='LoD signal')
            plt.axvline(lod_conc, linestyle='--',
                        label=f'LoD conc = {lod_conc:.2f}')
            ...
            if save_path:
                ...
                if have_samples:
                    ...
                    for n, s, c in zip(sample_names, sample_signals, sample_concs):
                        ws.append([n, s, c])
                    ws.append([])
                ...
                wb.save(save_path)
                messagebox.showinfo('Done', f'Results saved to {save_path}')

        except RuntimeError:
            messagebox.showwarning(
                "Fit Warning",
                "Optimal parameters not found (maxfev reached). "
                "Consider choosing a different analysis method."
            )
